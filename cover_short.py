...
def place_cover_order(symbol: str, quantity: int, limit_price: float):
    """
    ...
    Args:
        ...
        limit_price (float): Limit price for the cover order
    """
    # Initialize client
    client = Client(
        os.getenv('appKey'),
    ...
    try:
        ...
        print("\nAttempting to place cover order...")
...
if __name__ == "__main__":
    ...
    print(f"\nOrder Summary:")
    print(f"Cover {quantity} shares of {symbol} at ${limit_price:.2f}")
    # confirm = input("\nDo you want to place this order? (yes/no): ").lower()
    confirm = 'yes'
    if confirm == 'yes':
        place_cover_order(symbol, quantity, limit_price)
    else:
